...
def func(record):
    if record == 0: #A가 1 (바위 일 경우 funcB에서 record가 0이면 return 1
        return 1
    elif record == 1:  #A가 2 (보 일경우 functionB에서 record가 1이면 return 2)
        return 2
    return 0

def solution(recordA, recordB):   # 0 : 가위 , 1 : 바위 , 2 : 보
    cnt = 0
